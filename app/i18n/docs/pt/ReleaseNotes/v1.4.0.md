Essa versão da picfightiton foi majoritariamente dedicada a corrigir alguns
...
suporte inclui a possibilidade de criar transações na picfightiton e então
...
da picfightiton e nessa versão adicionamos uma prova de conceito para temas
...
- Adicionada primeira versão da integração com dispositivos Trezor. Isso permite
  que os usuários utilizem a picfightiton como uma carteira apenas de visualização
...
- PicFightiton foi modificada para utilizar a versão 2 do serviço de compra
...
- Quando um pânico é detectado pela picfightiton no pfcd ou pfcwallet, usuários
...
- A taxa de transação mínima padrão foi atualizada no pfcd a algumas versões e
  ...
  transação ou ticket criada no decrediton ou pfcwallet deve usar essa taxa
  mínima agora.
...
- Logs da picfightiton foram adicionadas na aba Logs, na página de ajuda.
...
- Transações agora são decodificadas dentro da picfightiton sem precisar serem
